...
* В случае, когда game -- идентификатор игры, а не пустая строка, а при этом пользователь, чей sid передается в запросе, не находится ни в одной игре либо game не совпадает с идентификатором игры, в которой он сейчас состоит, следует вернуть ошибку badGame
