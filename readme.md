...
* [PSG-protocol (Platform shooter game protocol)](#psg-protocol-platform-shooter-game-protocol)
    ...
    * [Websocket](#websocket)
        ...
        * [Использование респаунов и телепортов](#Использование-респаунов-и-телепортов)
            ...
            * [Пример использования телепортов](#Пример-использования-телепортов)
...
* websocket_mode => "synchron" / "simple"
...
* При телепортации игрока его скорость остается неизменной, а координаты изменяются на координаты центра телепорта.
* Каждая сеть телепортов помечается одной цифрой. Телепортов в сети может быть несколько. Если игрок зашел в телепорт, помеченный какой-то цифрой, выйти он должен из следующего телепорта, помеченного этой же цифрой, в порядке обхода
    - Из телепорта игрок выходит без потери энергии (скорость сохраняется)
...
#### Пример использования телепортов
...
* Игрок, зашедший в телепорт 1 во второй клетке первой строки выйдет из телепорта 1 в одиннадцатой клетке первой строки
* При входе в телепорт 1 в одиннадцатой клетке первой строки выход будет из телепорта 1 на нижней платформе
* При входе в телепорт 1 на нижней платформе выход будет из левого верхнего телепорта 1
* Аналогично с телепортами 2. Так как их всего два, вход и выход здесь очевидны

### Синхронный режим

* После получения запроса "startTesting" с параметром "websocket_mode" равным "synchron" включается синхронный режим работы websocket сервера.
...
* Каждый раз после отправки состаяния игры, считается, что ни один игрок не отправлял никаких запросов.
* Для получения нового состояния игры и симуляции "бездейстия" клиенту нужно отправить "empty" запрос.
