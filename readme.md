...
### Ответ (Response)

    {
        ...
        // Необязательный текстовый комментарий. Может быть произвольным
        "message": "<message>"
    }

* В случае успешного выполнения команды значение поля result должно быть равно "ok"
* В случае ошибки передается ее код. Если ошибок несколько, то передается набиолее приоритетная из них.
Ошибки с одинаковым приоритетом выбираются произвольно
...
* Ошибку badRequest следует отправлять в том случае, когда запрос соответствует стандарту JSON, но при этом не соответствует формату и не подходит ни одна ошибка типа bad<Property>. *Например, если какого-либо обязательного поля не было в сообщении*
...
* messages => массив, отсортированный *по возрастанию* времени получения сообщения на сервере
